...
const COOKIE_NAME = "cejop_encuesta_e2_done";
const LS_KEY = "cejop_encuesta_e2_completada";
...
function markE2Completed() {
  if (typeof window !== "undefined") localStorage.setItem(LS_KEY, "true");
}

function isE2Completed(): boolean {
  if (typeof window === "undefined") return false;
  return localStorage.getItem(LS_KEY) === "true";
}

export default function EncuestaE2Page() {
  ...
  useEffect(() => {
    ...
    if (cookie || isE2Completed()) setAlreadyDone(true);
  ...
  const canAdvance = (): boolean => {
    switch (currentStep) {
      ...
      case 3:
        return !!(form.dirigenteTucDisgustar.trim() && form.dirigenteArgDisgustar.trim());
      case 4:
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      encuentroId: "e2",
      fueAlPrimero: form.fueAlPrimero === "si",
      personal: {
          ...
          <AnimatePresence mode="wait">
            {encuestaCerrada ? (
              ...
              >
                ...
                <h2 className="font-montserrat font-black text-2xl sm:text-3xl text-white mb-5 leading-tight">
                  Las inscripciones ya están cerradas
                </h2>
                <p className="font-source text-white/80 text-[15px] leading-relaxed max-w-sm mx-auto mb-5">
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-white/10 text-white/70 hover:bg-white/20 hover:text-white transition-colors"
              ...
              <motion.div
                ...
                className="text-center py-12"
              >
                <CheckCircle size={56} className="text-cejop-blue-light mx-auto mb-6" />
                ...
                <p className="font-source text-white/80 text-base leading-relaxed max-w-sm mx-auto mb-4">
                  {duplicateMessage ||
                    "Ya recibimos tu encuesta para el 2do CEJOP. No es necesario completarla nuevamente."}
                </p>
              ...
              >
                ...
                <p className="font-source text-white/80 text-base leading-relaxed max-w-sm mx-auto mb-4">
                  Gracias por tomarte el tiempo de participar. El equipo de CEJOP
                  va a evaluar todos los perfiles recibidos para el 2do encuentro.
                </p>
                <p className="font-source text-white/50 text-sm leading-relaxed max-w-sm mx-auto">
                ...
                {isIntro ? (
                      ...
                      <div className="border-l-2 border-white/20 pl-4">
                        <p className="font-source text-[15px] text-white/80 leading-relaxed">
                          ...
                          <strong className="text-white">
                            únicamente para el segundo encuentro
                          </strong>
                          , el sábado 30 de mayo. Si ya participaste del primero,
                          igual tenés que volver a anotarte.
                        </p>
                      </div>

                      <div className="border-l-2 border-cejop-blue pl-4">
                        <p className="font-source text-[15px] text-white/80 leading-relaxed">
                          La capacidad del espacio es acotada, así que hacemos un
                          proceso de selección para cada encuentro. Si quedás
                          seleccionado,{" "}
                          <strong className="text-white">
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                      {currentStep === 1 && (
                        <>
                          ...
                          <RadioGroup
                            label="¿Fuiste al primer CEJOP?"
                            name="fueAlPrimero"
                            value={form.fueAlPrimero}
                            onChange={(v) =>
                              setForm({ ...form, fueAlPrimero: v as "si" | "no" })
                            }
                            options={[
                              { value: "si", label: "Sí" },
                              { value: "no", label: "No" },
                        ...
                        <div className="space-y-3">
                          ...
                          {selectedTopics.map((topic, i) => {
                            ...
                            return (
                              <div key={topic.id}>
                                <button
                                  onClick={() => setExpandedTopic(isOpen ? null : `deep-${topic.id}`)}
                                    ...
                                    >
                                      <div className="border border-t-0 border-white/15 bg-white/[0.05] px-4 py-3 space-y-3">
                                        ...
                                        <textarea
                                          ...
                                          onChange={(e) => handlePriorityResponse(topic.id, e.target.value)}
                                          rows={2}
                    ...
                    <div className="flex items-center gap-3 mt-10">
                      ...
                      >
                        {submitting ? "Enviando..." : isLastStep ? "Enviar inscripción" : "Siguiente"}
                        {!submitting && (
                          <ArrowRight
                            size={16}
...
function RadioGroup({
  ...
  return (
    <div>
      <span className="block font-encode text-[11px] font-semibold tracking-[0.2em] uppercase text-white/50 mb-2">
        {label}
        {required && <span className="text-cejop-blue-light ml-1">*</span>}
      </span>
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-3.5 border font-montserrat font-bold text-sm tracking-wide transition-all ${
                selected
                  ? "border-cejop-blue bg-cejop-blue/20 text-white"
