...
import { colName } from "@/lib/encuentro-config";

const ENCUENTRO_ID = "e2" as const;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const collection = db.collection(colName("feedback", ENCUENTRO_ID));
      ...
      return NextResponse.json(
        {
          duplicate: true,
          message:
            "Ya recibimos tu feedback sobre el segundo encuentro. Tu respuesta quedó registrada correctamente.",
        },
        { status: 409 }
      );
    }
    ...
    try {
      const encuesta = await db
        ...
        .findOne({ encuentroId: ENCUENTRO_ID, "personal.mail": mail });
      const nombre = encuesta?.personal?.nombre || "";
      await sendGraciasFeedback({ mail, nombre, encuentroId: ENCUENTRO_ID });
    } catch (err) {
    ...
    console.error("Error saving feedback-e2:", error);
    return NextResponse.json(
