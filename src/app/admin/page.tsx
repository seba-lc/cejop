...
function TabEmails() {
  ...
  const [testTipo, setTestTipo] =
    useState<EmailItem["tipo"]>("inscripto_no_confirmado");
  ...
  const fetchData = useCallback(async () => {
    ...
    if (!res.ok) {
      setErrorMsg("No se pudo cargar la lista de destinatarios");
      return;
    }
    const data = await res.json();
    setItems(data.items || []);
  ...
  async function sendTest() {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(testMail.trim())) {
      setErrorMsg("Ingresá un email válido para el test");
      return;
    }
    setSendingTest(true);
    setStatusMsg(null);
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setErrorMsg(data.error || "Error al enviar el test");
      } else {
        setStatusMsg(`Test enviado a ${testMail}`);
      }
    } catch {
      setErrorMsg("No se pudo enviar el test");
    } finally {
  ...
  async function sendMass() {
    if (!counts || counts.pendientes === 0) return;
    const confirm = window.confirm(
      `Vas a enviar el email a ${counts.pendientes} destinatarios. ¿Continuar?`
    );
    if (!confirm) return;

    setSendingMass(true);
    setStatusMsg("Enviando... esto puede tardar un minuto.");
    setErrorMsg(null);
    try {
      const res = await fetch("/api/admin/send-email", {
        method: "POST",
      ...
      const data = await res.json();
      if (!res.ok) {
        setErrorMsg(data.error || "Error en el envío masivo");
      } else {
        setStatusMsg(
          `Listo. Enviados: ${data.sent}. Fallidos: ${data.failed}.`
        );
        await fetchData();
      }
    } catch {
      setErrorMsg("No se pudo completar el envío masivo");
    } finally {
  ...
  return (
    <div className="space-y-6">
      <SectionTitle>Emails — Primer encuentro</SectionTitle>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card>
          <div className="flex items-center gap-2 mb-3">
            <Users size={16} className="text-cejop-blue" />
            <span className="text-sm text-gray-400 font-medium">Asistentes</span>
          </div>
          <p className="text-3xl font-bold text-white font-montserrat">
            {counts?.total || 0}
          </p>
      ...
      <Card>
        ...
        <p className="text-sm text-gray-400 mb-4">
          Mandate una versión de prueba a tu email para verificar que el
          template se ve bien antes del envío masivo.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-3">
          ...
          <select
            value={testTipo}
            onChange={(e) => setTestTipo(e.target.value as EmailItem["tipo"])}
          ...
          >
            <option value="confirmado">Confirmado (tipo A)</option>
            <option value="inscripto_no_confirmado">Inscripto (tipo A)</option>
            <option value="walk_in">Walk-in (tipo B)</option>
          </select>
          <button
            onClick={sendTest}
            disabled={sendingTest}
            className={`flex items-center justify-center gap-2 px-5 py-2.5 bg-cejop-blue text-white text-sm font-medium rounded-lg transition-colors ${
              sendingTest ? "opacity-60" : "hover:bg-cejop-blue/90"
            }`}
          >
            <Send size={14} />
      ...
      <Card>
        <div className="flex items-start justify-between gap-4 flex-col sm:flex-row">
          <div>
            ...
            <p className="text-sm text-gray-400">
              Se enviará el email correspondiente según el tipo de cada
              asistente (inscriptos reciben versión A, walk-ins reciben
              versión B). Los ya enviados se omiten automáticamente.
            </p>
          </div>
          <button
            onClick={sendMass}
            disabled={sendingMass || !counts || counts.pendientes === 0}
            className={`shrink-0 flex items-center justify-center gap-2 px-5 py-3 text-sm font-semibold rounded-lg transition-colors ${
              counts && counts.pendientes > 0 && !sendingMass
          ...
          >
            ...
            {sendingMass
              ? "Enviando..."
              : `Enviar a ${counts?.pendientes || 0} destinatarios`}
          </button>
        </div>
      </Card>
      ...
      <Card className="!p-0 overflow-hidden">
        <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-400">
            <span>Tipo A confirmado: {counts?.confirmado || 0}</span>
            <span className="hidden sm:inline">
              Tipo A inscripto: {counts?.inscripto_no_confirmado || 0}
            </span>
            <span>Tipo B walk-in: {counts?.walk_in || 0}</span>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((it) => (
                ...
                >
                  ...
                  <td className="px-6 py-3 text-gray-400">{it.mail}</td>
